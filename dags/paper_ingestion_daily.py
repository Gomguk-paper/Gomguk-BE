...
with DAG(
    dag_id="paper_pool_ingestion",
    start_date=datetime(2026, 2, 1),
    schedule="@daily",  # 필요 시 변경
    catchup=False,
    default_args={
        "owner": "airflow",
        "retries": 2,
    ...
    tags=["ingestion", "paper_pool"],
) as dag:

    @task(task_id="pull_arxiv_recent")
    def pull_arxiv_recent_task():
        hook = PostgresHook(postgres_conn_id="db")
        pull_arxiv_recent_into_paper_pool(hook=hook, max_results_per_category=2000)
    ...
    pull_arxiv_recent_task() >> pull_influential_citation_task() >> pull_citation_task()
